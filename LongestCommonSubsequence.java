...
public class LongestCommonSubsequence {
    ...
    public static void main(String[] args) {
        ...
        // Find the longest common subsequence of the two input words
    ...
    public static String findLCS(String stringOne, String stringTwo) {
        ...
        for (int i = 0; i < stringOne.length(); i++) {
            for (int j = 0; j < stringTwo.length(); j++) {
                // If the current characters match, the length of the common subsequence increases by 1
                if (stringOne.charAt(i) == stringTwo.charAt(j)) {
                    lengths[i + 1][j + 1] = lengths[i][j] + 1;
                } else {
                    // Otherwise, the length of the common subsequence is the maximum of the lengths of the subsequences without the current character
        ...
        for (int i = stringOne.length(), j = stringTwo.length(); i!= 0 && j!= 0;) {
            ...
            // Otherwise, the characters at the current positions match, so append the character to the LCS string and move diagonally up-left
            else {
